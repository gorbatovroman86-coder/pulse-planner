...
export function TaskLine({
  ...
  const meta = (
    <>
      {task.due_date && (
        <span
          className="num shrink-0 text-[12.5px]"
      ...
      {task.estimate_minutes > 0 && (
        <span className="num shrink-0 text-[12.5px] text-ink3">{fmtHours(task.estimate_minutes)} ч</span>
      )}
    </>
  )
  ...
  return (
    <div
      ...
      data-task={task.id}
      className={`group flex items-center gap-2.5 rounded-lg px-2 ${dense ? 'py-1.5' : 'py-2'} ${
        landing ? 'landing' : ''
      } ${selected ? 'bg-sunken' : 'hover:bg-hover'} ${draggable ? 'cursor-grab active:cursor-grabbing' : ''}`}
      style={
        task.priority === 1 && !done
          ? { boxShadow: 'inset 2px 0 0 var(--color-alarm)' }
          : undefined
      }
    >
      ...
      <div className="flex shrink-0 items-center gap-2.5 group-hover:hidden">{meta}</div>
