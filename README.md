...
Текущее состояние и следующий шаг — в [PROJECT.md](./PROJECT.md).
...
Направление — **спокойный прибор**: белые карточки на холодной бумаге, иерархия задаётся
весом и воздухом, а не сеткой линеек. Один акцент — тёмно-зелёный: чекбокс, главная кнопка,
активная вкладка, линия «сохранено». Тревожный цвет отдан ровно одному значению: «остыл».
Моноширинный шрифт — только на числах.

Первая версия была набрана как бухгалтерская ведомость: хайрлайны вместо иерархии,
десятипиксельный капслок на каждой подписи, строки по 42 px. Она читалась как отладочная
консоль, а не как продукт, и была переделана после разбора Todoist, Linear и Things:
круглые чекбоксы, подписи обычным регистром, строки 56 px, текст 14,5 px, кнопки трёх видов.

Отвергнуты два направления:

- **тёплая бумага с контрастной антиквой и терракотой** — это то, что нейросеть выдаёт
  по умолчанию на любой бриф;
- **тёмный пульт с одним кислотным акцентом** — девять цветов проектов на тёмном фоне
  приходится делать яркими, и экран превращается в гирлянду.

Подробности решений и оценки по пяти критериям — в [DECISIONS.md](./DECISIONS.md).

Шрифты: **Geologica** (текст, заголовки) и **JetBrains Mono** (числа, даты, длительности).
Оба лежат в сборке — интернет для отрисовки не нужен.
...
- **Список проектов по зонам жизни:** 🟢 В работе · ⚠️ На грани · 💀 Кладбище.
  Проект проваливается вниз сам, как только перестаёшь закрывать его задачи,
  и всплывает обратно, когда снова берёшься. Переезд строки показан движением:
  вниз она падает тяжелее, чем поднимается.
- **Инфографика смертей** в каждой строке: столбики — закрытые за день часы,
  под ними непрерывная линия жизни. Красные участки — периоды, когда проект был мёртв,
  рядом счётчик «💀 3» — столько раз он умирал за три недели. Проект, который в окне
  ещё не работал, красным не красится: мёртвым считается только то, что жило.
- **Справа честный срок:** «умрёт завтра», «умрёт через 3 дн.», «мёртв 9 дней».
- **Подсказчик** в развороте строки разбирает состояние и предлагает конкретный
  следующий шаг: нет задач, есть просрочка, задача весит три часа, бюджет не выбран.
  Предложенную задачу можно создать одной кнопкой.
- **Лента трёх недель** — столбик на каждый день, высота равна закрытым за день часам.
  Живёт в развороте проекта и на экране проекта, а не в общем списке.
- **Порог смерти — два дня.** Если за это время ни одна задача не закрыта и касаний
  не было, проект уезжает в 💀. Порог правится у каждого проекта отдельно.
  Проект без единой открытой задачи получает метку «нет задач», даже если касание было вчера.
- **Значок проекта** ставится на экране проекта и стоит в строке слева;
  в зоне «Кладбище» он обесцвечивается.
...
- **Колонка «Сегодня»** справа: сколько набрано против длины твоего дня, полоса
  заполнения (тёмное — уже сделано, светлое — предстоит), перетаскивание для порядка
  и честное «перебор 1 ч 30 мин — день столько не вместит». Длина дня правится прямо
  здесь, кликом по «8 ч», и уезжает в облако вместе с остальным.
- **Входящие** внизу — задачи без проекта, с выпадающим списком для раскладки по проектам.

**Время, срок и важность правятся прямо в строке задачи.** Наведи на задачу — появятся
«+ время», «+ срок», «+ важность»; заполненные видны всегда. Время принимает и голое
число (минуты), и «90м», и «1,5ч». Важность переключается кликом: 1 → 2 → 3.
...
- **Длина рабочего дня** — сколько часов реально помещается (по умолчанию 8).
  Задачи «на сегодня» меряются против неё, и перебор виден сразу.
