...
- **Доска стикеров** — проекты разложены по степени жизни: 🔥 Горит · 🌡️ Стынет ·
  💀 Умер · 🌱 Ещё не начат. Стикер несёт значок проекта, цветную кромку, число задач,
  дни без касания и полосу «неделя против бюджета». Значок проекта меняется кликом.
  Есть переключатель **Доска / Список** — старая ведомость никуда не делась.
- **Перетаскивание стикера в «Горит»** отмечает касание. В другие полосы перетащить
  нельзя: состояние считается из закрытых задач, руками его не поставить — приложение
  так и скажет.
- **Подсказчик** в развороте проекта разбирает состояние и предлагает конкретный
...
- **Ненавязчивые напоминания:** значок состояния на стикере тихо живёт — пламя мерцает,
  росток проклёвывается, мёртвый стикер лежит криво и выцвел. Самый забытый проект
  раз в десять секунд расходится тихим кольцом. Всё выключается системной настройкой
  «уменьшить движение».
